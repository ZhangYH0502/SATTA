...
from utils1 import lr_per_category, topk_selection
...
def model_inference_with_salr(args, net, dataloader):
    ...
    for batch in tqdm(dataloader, mininterval=0.5, desc='Validating', leave=False, ncols=50):
        ...
        results, parameters_list = salr_train(net, images, parameters_list, cls=args.num_labels-1)
    ...
    loss_total = np.array([0])

    print('\n')
    print('output test metric:')
    test_metrics = utils.compute_metrics(all_predictions, all_targets, loss_total)
...
def salr_train(net, images, parameters_list, cls=3):
    ...
    for si in range(cls):
        ...
        net.load_state_dict(parameters_list[sii])

        pred, uncertainty_map, feature_proj, proxy_proj = net(images.cuda())
        pred_map = torch.argmax(pred)
        results.append((pred_map == sii).astype(int))

        cls_list = torch.unique(pred_map)

        if sii in cls_list:
            lr_i = lr_per_category(uncertainty_map, pred_map, cal=sii, alpha=0.001)
            feas, labs = topk_selection(feature_proj, uncertainty_map, pred_map, cls_list, cal=sii, k=20)

            optimizer = torch.optim.Adam(net.parameters(), lr=lr_i)

            criterion_proxy = lf.ProxyPLoss_Single(num_classes=2, scale=12)
            loss = criterion_proxy(feas, labs, proxy_proj[0, :, :], sii)

            loss.backward()
            optimizer.step()
            optimizer.zero_grad()

            parameters_list[sii] = net.state_dict()
