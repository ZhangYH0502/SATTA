...
def lr_per_category(uncertainty_map, pred_map, cal=1, alpha=0.1):
    ...
    return alpha * lr.clone().detach().item()
...
def topk_selection(feature_map, uncertainty_map, pred_map, cal_list, cal=1, k=20):
    ...
    positive_feas = torch.stack(positive_feas, dim=0)
    positive_feas = positive_feas.contiguous().view(positive_feas.shape[0] * positive_feas.shape[1],
                                                    positive_feas.shape[2])
    positive_labs = torch.ones(positive_feas.shape[0])

    negative_feas = torch.stack(negative_feas, dim=0)
    negative_feas = negative_feas.contiguous().view(negative_feas.shape[0] * positive_feas.shape[1],
                                                    negative_feas.shape[2])
    negative_labs = torch.zeros(negative_feas.shape[0])
